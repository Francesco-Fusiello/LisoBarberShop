<x-layout>
    <section class="py-5" style="background-color: #0b2e1d; font-family: 'Playfair Display', serif; color: #fff;">
        <div class="container">
            <h1 class="text-center mb-4" style="font-size: 3.5rem; font-weight: 700; letter-spacing: 2px; color: #d4af37;">
                Contattaci
            </h1>
            
            <!-- Mappa -->
            <div class="row justify-content-center mt-4">
                <div class="col-md-8">
                    <div class="embed-responsive embed-responsive-16by9" style="height: 400px;">
                        <iframe 
                        src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3068.926922891084!2d16.292020!3d41.216378!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x133a2f3e3d6ff927%3A0x5640eb67d88b8a8d!2sViale%20Pietro%20Nenni%2C%20324%2C%2041200%20Andria%2C%20Italia!5e0!3m2!1sit!2sit!4v1627057125456!5m2!1sit!2sit" 
                        frameborder="0" 
                        style="height: 100%; width: 100%;" 
                        allowfullscreen>
                    </iframe>
                </div>
            </div>
        </div>
        
        <p class="text-center fs-5 pt-5" style="color: #ccc;">
            Scarica la nostra app per avere sempre Liso Barber Shop a portata di mano. Prenota facilmente il tuo appuntamento e scopri tutte le novità!
        </p>
            <!-- Badge App Store & Google Play -->
            <div class="row justify-content-center mt-4">
    <div class="col-md-8">
        <div class="d-flex justify-content-center align-items-center gap-4 flex-wrap mt-4">
            <!-- App Store Badge -->
            <a href="https://apps.apple.com/it/app/liso-barber-shop/id6502577739" target="_blank">
                <img src="https://tools.applemediaservices.com/api/badges/download-on-the-app-store/black/it-it?size=250x83"
                     alt="Scarica su App Store"
                     style="height: 60px; max-width: 100%; object-fit: contain;">
            </a>

            <!-- Google Play Badge -->
            <a href="https://play.google.com/store/search?q=liso+barber+shop&c=apps&hl=it" target="_blank">
                <img src="https://play.google.com/intl/en_us/badges/static/images/badges/it_badge_web_generic.png"
                     alt="Disponibile su Google Play"
                     style="height: 78px; max-width: 100%; object-fit: contain;">
            </a>
        </div>
    </div>
</div>


            <!-- Dettagli su come contattarci -->
            <div class="row justify-content-center mt-5">
                <div class="col-md-6">
                    <h3 style="color: #d4af37;">Dove trovarci</h3>
                    <p style="color: #ccc;">Liso Barber Shop, Viale Pietro Nenni 324, Andria, IT</p>

                    <h3 style="color: #d4af37;">Orari di apertura</h3>
                    <ul style="color: #ccc;">
                        <li>Lunedì - Giovedì: 08:00/13:00 - 15:00/20:00</li>
                        <li>Venerdì - Sabato: 08:00 - 20:00</li>
                        <li>Domenica: Chiuso</li>
                    </ul>

                    <h3 style="color: #d4af37;">Contattaci</h3>
                    <p style="color: #ccc;">Telefono: +39 123 456 789</p>
                    <p style="color: #ccc;">Email: <a href="mailto:info@lisobarbershop.it" style="color: #d4af37;">info@lisobarbershop.it</a></p>
                </div>
            </div>
        </div>
    </section>
</x-layout>
